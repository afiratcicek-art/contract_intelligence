...
@router.get("/{doc_id}/relations", status_code=200)
def get_document_relations(
    project_id: str,
    doc_id: str,
    access=Depends(verify_project_access),
):
    """Return document_relations rows for a given doc,
    enriched with target pdf_document metadata.

    Two-step fetch — no N+1:
      1. All relation rows for source doc_id (project-scoped).
      2. All target docs in a single IN query.
    Response: list of relation dicts with nested 'target' key.
    """
    db = access["db"]
    try:
        # Step 1: fetch relations (RLS enforced via JWT client)
        rel_res = (
            db.table("document_relations")
            .select(
                "target_doc_id, score, score_breakdown, "
        ...
        relations: list[dict] = rel_res.data or []

        if not relations:
            return []

        # Step 2: fetch target doc details (single IN query)
        ...
        doc_map: dict[str, dict] = {
            d["id"]: d for d in (doc_res.data or [])
        }

        # Step 3: merge and return
        enriched = []
        for rel in relations:
            target = doc_map.get(rel["target_doc_id"])
            if target is None:
                continue          # orphan — skip silently
            enriched.append(
                {
        ...
        logger.error(
            "Belge ilişki listesi hatası: %s | doc=%s project=%s",
            exc, doc_id, project_id,
        )
        raise HTTPException(
            status_code=500, detail="Belge ilişkileri alınamadı."
        )
